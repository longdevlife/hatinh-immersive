...
page.on('response', async (response) => {
  ...
  if (
    /xml|json|text/i.test(contentType) &&
    /snap|skin|tour|scene|project|group|360|krpano/i.test(url)
  ) {
    try {
      const text = await response.text();
  ...
  const dom = await page.evaluate(() => {
    ...
    for (const el of [...document.querySelectorAll('*')]) {
      ...
      const visible =
        ...
        style.display !== 'none';
      const interesting =
        /download|tải|camera|snap|screen|save|export|image|ảnh|check[- ]?in|chụp/i.test(
          `${text} ${aria ?? ''} ${title ?? ''} ${href ?? ''} ${onclick ?? ''} ${className} ${src ?? ''}`,
        );
      const clickable =
    ...
    const globals = Object.keys(window).filter((key) =>
      /krpano|snap|download|save|camera|checkin/i.test(key),
    );
    return {
      ...
      handleKrpanoScreenshot:
        typeof window.handleKrpanoScreenshot === 'function'
          ? window.handleKrpanoScreenshot.toString().slice(0, 12000)
          : null,
...
for (const [url, text] of jsResponses) {
  ...
  for (const token of [
    ...
    if (index >= 0)
      snippets.push(text.slice(Math.max(0, index - 600), Math.min(text.length, index + 1800)));
  }
...
for (const item of textResponses) {
  const matched = patterns
    ...
    .map((pattern) => pattern.source);
  if (matched.length)
    matches.push({ url: item.url, matched, snippets: [item.text.slice(0, 10000)] });
}

const allRows = [...landing.rows, ...tour.rows, ...menu.rows];
const downloadLike = (row) =>
  row.visible &&
  /download|tải|camera|snap|save|export|check[- ]?in|chụp/i.test(
    `${row.text} ${row.aria ?? ''} ${row.title ?? ''} ${row.href ?? ''} ${row.onclick ?? ''} ${row.className} ${row.src ?? ''}`,
  );
const summary = {
  url: URL,
  enteredTour:
    /toàn cảnh|bản đồ|thông tin|thiên cầm/i.test(tour.bodyText) ||
    !!tour.globals.find((key) => /krpano/i.test(key)),
  menuClick,
  jsResponses: jsResponses.size,
  matchedResources: matches.length,
  visibleDownloadLikeDom: allRows.filter(downloadLike).length,
  explicitDownloadAnchors: allRows.filter(
    (row) =>
      row.visible && row.tag === 'a' && /download|tải/i.test(`${row.text} ${row.href ?? ''}`),
  ).length,
